<template>
  <mu-list :nested-indent="false" toggle-nested>
    <mu-list-item button :ripple="false" nested :open="open === 'send'" @toggle-nested="open = arguments[0] ? 'send' : ''">
      <mu-list-item-action>
        <mu-icon class="toggle-icon" size="24" :value="open === 'send'? 'keyboard_arrow_down':'keyboard_arrow_right'"></mu-icon>
      </mu-list-item-action>
    ...
    <mu-list-item button :ripple="false" nested :open="open === 'drafts'" @toggle-nested="open = arguments[0] ? 'drafts' : ''">
      <mu-list-item-action>
        <mu-icon class="toggle-icon" size="24" :value="open === 'drafts'? 'keyboard_arrow_down':'keyboard_arrow_right'"></mu-icon>
      </mu-list-item-action>
      <mu-list-item-title>我的好友</mu-list-item-title>
      <mu-list-item button v-for="f in friendsList" @click="showDetail(f._id)" class="f-item" :key="f._id" slot="nested">
        <mu-list-item-action>
